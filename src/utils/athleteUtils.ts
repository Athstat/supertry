...
export function formatAction(actionName: string) {
  const displayName = actionName.replace(/([A-Z])/g, ' $1').trim();
...
export function convertPositionNameToPositionObject(positionToSwap: string) {
  return {
    ...
    shortName: positionToSwap === 'any' ? 'ANY' : positionToSwap.substring(0, 2).toUpperCase(),
    x: '0',
    y: '0',
  };
}
...
export function athletePositionFilter(
  athletes: IProAthlete[],
  selectedPositions: string[] | undefined
): IProAthlete[] {
  const formatted =
    selectedPositions === undefined ? undefined : selectedPositions.map(p => formatPosition(p));
  ...
  return filtered.filter(a => {
    ...
    const matchesPositionClass =
      hasPositionClass && formatted.includes(formatPosition(positionClass));

    return matchesPosition || matchesPositionClass;
  });
}

export function athleteTeamFilter(
  athletes: IProAthlete[],
...
export function athleteSorter(
  athletes: IProAthlete[],
  sortType: SortField | undefined,
 ...
 */
export function isPowerRatingBest(athlete: IProAthlete, comparePlayers: IProAthlete[]): boolean {
  let maxVal: number = 0;
