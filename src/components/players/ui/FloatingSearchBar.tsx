...
export default function FloatingSearchBar({
  ...
  return (
    ...
    >
      ...
      >
        <Search className="w-5 h-5 text-slate-800 dark:text-white/90" />
        <input
          type="text"
          value={value}
          onChange={e => onChange(e.target.value)}
          placeholder={placeholder}
          className="flex-1 bg-transparent outline-none text-sm md:text-base text-slate-800 dark:text-white/90 placeholder:text-slate-800 dark:placeholder:text-white/90"
        />
      </div>
      <button
