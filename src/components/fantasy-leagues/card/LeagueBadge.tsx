        ...
        if (leagueGroup.type === "official_league" || leagueGroup.type === "system_created") {
            return <Badge variant="official" >
                <div className='flex flex-row items-center gap-2' >
                    <LeagueGoldCheckMark fill={isDark ? "black" : "#BF9701"} checkFill="transparent" leagueGroup={leagueGroup} />
                    <p>Official</p>
                </div>
            </Badge>
        }

        if (leagueGroup.type === "celebrity_created") {
            return <Badge variant="celebrity" >
                <div className='flex flex-row items-center gap-1' >
                    <LeagueGoldCheckMark fill={isDark ? "white" : "#5A30EF"} checkFill='transparent' leagueGroup={leagueGroup} />
                    <p>Official</p>
                </div>
            </Badge>
        }

        if (leagueGroup.type === "subscriber_created") {
            return <Badge variant="subscriber" >
                <div className='flex flex-row items-center gap-1' >
                    <LeagueGoldCheckMark fill={isDark ? "white" : "#1196F5"} checkFill='transparent' leagueGroup={leagueGroup} />
                    <p>Official</p>
                </div>
            </Badge>
        }
...
const Badge = ({ variant, children }: { variant: string; children: React.ReactNode }) => {
    const getVariantClasses = () => {
        switch (variant) {
            ...
            case 'invite':
                return 'bg-slate-200 text-purple-800 dark:bg-purple-600/70 dark:text-purple-200';
            case 'official':
                return 'bg-[#F2EDDC] text-[#BF9701] dark:bg-[#BF9701] dark:text-[#000000]';
            case 'celebrity':
                return 'bg-[#E6E2F5] text-[#5A30EF] dark:bg-[#5A30EF] dark:text-[#FEFEFE]';
            case 'subscriber':
                return 'bg-[#DEEBF5] text-[#1196F5] dark:bg-[#1196F5] dark:text-[#FEFEFE]';
            default:
    ...
    return (
        <div className={`px-2 py-0.5 text-xs rounded-full ${getVariantClasses()}`}>
            {children}
        </div>
    );
};
...
export function LeagueGoldCheckMark({ leagueGroup, size = "13", checkFill, fill, clickable }: GoldCheckMarkProps) {

    const isOfficial = isLeagueOfficial(leagueGroup);
    const { openTooltipModal } = useTooltip();

    const finalCheckFill = checkFill ? checkFill :
        leagueGroup?.type === "celebrity_created" ? "white" :
        leagueGroup?.type === "subscriber_created" ? "white" :
        "#14171A"

    const finalFill = fill ? fill :
        leagueGroup?.type === "celebrity_created" ? "#5A30EF" :
        leagueGroup?.type === "subscriber_created" ? "#1196F5" :
        "#DDAE00"
