...
interface TableHeaderProps {
  ...
  sortOrder: 'asc' | 'desc';
  onSort: (field: 'price' | 'rating' | 'attack' | 'defense' | 'kicking') => void;
}

const TableHeader: React.FC<TableHeaderProps> = ({ sortBy, sortOrder, onSort }) => {
  // Helper to render sort indicator
  const renderSortIndicator = (field: 'price' | 'rating' | 'attack' | 'defense' | 'kicking') => {
  ...
  return (
    <div className="flex items-center px-4 py-3 bg-gray-100 dark:bg-dark-700 border-y dark:border-gray-700 text-sm font-semibold text-gray-700 dark:text-gray-300">
      ...
      >
        PTS {renderSortIndicator('price')}
      </button>
      
      <button 
        onClick={() => onSort('rating')}
        className={`w-12 text-center flex items-center justify-center transition ${sortBy === 'rating' ? 'text-green-600' : ''}`}
      >
      ...
      <button 
        ...
        className={`w-16 text-center flex items-center justify-center transition ${sortBy === 'attack' ? 'text-green-600' : ''}`}
      >
      ...
      <button 
        ...
        className={`w-16 text-center flex items-center justify-center transition ${sortBy === 'defense' ? 'text-green-600' : ''}`}
      >
      ...
      <button 
        ...
        className={`w-16 text-center flex items-center justify-center transition ${sortBy === 'kicking' ? 'text-green-600' : ''}`}
      >
