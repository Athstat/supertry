...
export default function RelatedPlayersList({ player }: Props) {
  ...
  if (isLoading) {
    return (
      <div>
        <RoundedCard className="border-none w-15 h-15 rounded-full bg-slate-200 dark:bg-slate-700 animate-pulse" />
        ...
        {teamMates.map(t => {
          return (
            <div className="flex flex-col items-center justify-center">
              ...
              {player.athstat_firstname && player.athstat_lastname && (
                <SecondaryText className="text-xs text-center truncate">
                  {t.athstat_lastname}
                </SecondaryText>
              )}
            </div>
